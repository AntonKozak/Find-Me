...
export default function MessageTable({ messages }: Props) {
  ...
  const isOutbox = searchParams.get('conteiner') === 'outbox';
    ...
    (item: MessageDto, columnKey: keyof MessageDto) => {
      ...
      switch (columnKey) {
        ...
        case 'senderName':
          return (
            <div
              className={`flex item-center gap-2 cursor-pointer ${!item.dateRead && !isOutbox ? 'font-semibold' : ''}`}
            >
              <Avatar
                alt='Image of member'
                src={
        ...
        case 'text':
          return (
            <div className='truncate'>{truncateString(cellValue, 90)}</div>
          );
